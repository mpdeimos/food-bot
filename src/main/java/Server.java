...
public class Server {
	...
	public Server() {
		...
		Spark.before((request, response) -> response.type("text/plain; charset=UTF-8"));
		...
		for (Entry<IBistro, IMenu> entry : menu) {
			try {
				...
				for (IDish dish : entry.getValue().getDishes()) {
					...
					writer.println(" " + config.PRICE_FORMAT.format(dish.getPrice()));
					writer.println(Strings.EMPTY);
				}
			} catch (Exception e) {
	...
	public Object getUrl(Request req, Response res) throws ClientProtocolException, IOException {
		res.type("text/html; charset=UTF-8");

		return org.apache.http.client.fluent.Request.Get(req.queryParams("url")).connectTimeout(5000)
